<?php
/**
 * Plugin Name: WooCommerce Menu Manager
 * Description: Reorganize the WooCommerce admin menu by moving Products, Payments, Analytics, and Marketing into the WooCommerce submenu. Fully customizable and multilingual.
 * Version: 1.0.0
...
if ( ! in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	add_action( 'admin_notices', function() {
		echo '<div class="error"><p><strong>WooCommerce Menu Manager</strong> ' . esc_html__( 'requires WooCommerce to be installed and active.', 'wc-menu-manager' ) . '</p></div>';
	} );
	return;
}
...
class WC_Menu_Manager {
	...
	public function define_menus() {
		$this->menus_to_move = array(
			'products' => array(
				'name'   => __( 'Products', $this->textdomain ),
				'remove' => 'edit.php?post_type=product',
				'add'    => 'edit.php?post_type=product',
				'cap'    => 'edit_products',
			),
			'payments' => array(
				'name'   => __( 'Payments', $this->textdomain ),
				'remove' => 'wc-settings&tab=checkout',
				'add'    => 'admin.php?page=wc-settings&tab=checkout',
				'cap'    => 'manage_woocommerce',
			),
			'analytics' => array(
				'name'   => __( 'Analytics', $this->textdomain ),
	...
	public function add_settings_link( $links ) {
		$settings_link = '<a href="options-general.php?page=wc-menu-manager-settings">' . esc_html__( 'Settings', $this->textdomain ) . '</a>';
	...
	public function add_plugin_page() {
		add_options_page(
			esc_html__( 'WC Menu Manager', $this->textdomain ),
			esc_html__( 'WC Menu Manager', $this->textdomain ),
			'manage_options',
	...
	public function create_admin_page() {
		?>
		<div class="wrap">
			<h1><?php echo esc_html__( 'WooCommerce Menu Manager', $this->textdomain ); ?></h1>
	...
	public function page_init() {
		...
		add_settings_section(
			'setting_section_id',
			esc_html__( 'Select menu items to move', $this->textdomain ),
...
class WC_Menu_Manager {
	...
	public function print_section_info() {
		echo esc_html__( 'Choose which menu items to move into the WooCommerce submenu:', $this->textdomain );
	}
	...
	public function create_checkbox_field( $args ) {
		...
		<input type="checkbox"
			...
			name="<?php echo esc_attr( $this->option_key ); ?>[<?php echo esc_attr( $id ); ?>]"
			value="1"
			<?php checked( isset( $options[ $id ] ) ? $options[ $id ] : 0, 1 ); ?> />
		<label for="<?php echo esc_attr( $id ); ?>"><?php echo esc_html__( 'Move into submenu', $this->textdomain ); ?></label>
		<?php
	}
